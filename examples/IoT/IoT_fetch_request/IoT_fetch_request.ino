...
/*************************************************************
  This is the example of fetching the data from the Connect Anduino server and displaying
  the response on the serial monitor (Note: this is just for fetching no operation based on
  the server response is done).
...
String roomAccessId = "YOUR_ROOM/DEVICE_ACCESSID"; //You will find the access id for room as well as device on the Connect Anduino android app IoT section
String url;
...
void loop() {
  String postData = "fingerprint="+fingerprint+"&accessid="+roomAccessId;  //Prepares the post data for fetching the data from the server
  http.begin(url);
